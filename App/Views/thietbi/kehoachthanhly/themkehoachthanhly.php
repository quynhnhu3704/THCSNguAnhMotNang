...
if(isset($_POST['btnluu'])) {
    ...
    if($maKeHoachThanhLy) {
        ...
        if($kq) {
            echo "<script>alert('Thêm kế hoạch thành công'); window.location.href='index.php?page=dskehoachthanhly'</script>";
        } else {
            $p->deleteKeHoachThanhLy($maKeHoachThanhLy); // rollback: xóa kế hoạch nếu thêm chi tiết thất bại
            echo "<script>alert('Thêm kế hoạch thất bại.'); window.location.href='index.php?page=themkehoachthanhly';</script>";
        }
    } else {
        echo "<script>alert('Thêm kế hoạch thất bại!'); window.location.href='index.php?page=themkehoachthanhly';</script>";
        exit();
