...
if(isset($_POST['btnluu'])) {
    ...
    if($p->checkName($tenNhaCungCap)) {
        echo '<script>alert("Tên nhà cung cấp này đã tồn tại. Vui lòng chọn tên khác."); window.history.back();</script>';
    } else {
        if($p->insertNhaCungCap($tenNhaCungCap, $diaChi, $soDienThoai, $email)) {
            echo '<script>alert("Nhà cung cấp đã được thêm thành công."); window.location.href="index.php?page=dsnhacungcap";</script>';
        } else {
            echo '<script>alert("Thêm nhà cung cấp không thành công. Vui lòng thử lại."); window.history.back();</script>';
        }
    }
}
?>
